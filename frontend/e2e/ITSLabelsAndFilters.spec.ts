...
test.describe('ITS Labels and Filters Tests', () => {
  ...
  test('label chips display in table rows', async ({ page }) => {
    ...
    const labelChips = page.locator('[class*="chip"], [class*="tag"], [class*="badge"]').filter({ hasText: /edge|default|location-group/ });
  ...
  test('clicking label chip filters table', async ({ page }) => {
    ...
    const labelChips = page.locator('[class*="chip"], [class*="tag"]').filter({ hasText: /edge|default/ });
    const chipCount = await labelChips.count();
  ...
  test('filter chips can be removed', async ({ page }) => {
    ...
    if (chipCount > 0) {
      ...
      if (await filterChip.isVisible()) {
        ...
        const removeButton = filterChip.locator('button, [class*="remove"], [class*="close"]').first();
  ...
  test('label editing dialog shows existing labels', async ({ page }) => {
    ...
    if (buttonCount > 0) {
      ...
      if (await editLabelsItem.isVisible()) {
        ...
        const existingLabels = ['cluster.open-cluster-management.io/clusterset', 'location-group', 'name'];
  ...
  test('new labels can be added in edit dialog', async ({ page }) => {
    ...
    if (buttonCount > 0) {
      ...
      if (await editLabelsItem.isVisible()) {
        ...
        const addLabelButton = page.getByRole('button').filter({ hasText: /Add Label|Add|New/i }).first();
        if (await addLabelButton.isVisible()) {
          ...
          if (isEnabled) {
            ...
            const valueInput = page.locator('input[placeholder*="value"], input[name*="value"]').last();
            ...
            if (await keyInput.isVisible() && await valueInput.isVisible()) {
              ...
              // Save changes
              const saveButton = page.getByRole('button').filter({ hasText: /Save|Update|Apply/i }).first();
  ...
  test('labels can be deleted in edit dialog', async ({ page }) => {
    ...
    if (buttonCount > 0) {
      ...
      if (await editLabelsItem.isVisible()) {
        ...
        const deleteButtons = page.locator('button[aria-label*="delete"], button[aria-label*="remove"], button').filter({ hasText: /×|delete|remove/i });
          ...
          if (isEnabled) {
            ...
            if (await keyInput.isVisible() && await valueInput.isVisible()) {
              ...
              if (await saveButton.isVisible()) {
                ...
                const errorMessage = page.locator('text=/invalid|error|spaces not allowed/i').first();
  ...
  test('bulk label editing works for multiple clusters', async ({ page }) => {
    ...
    if (checkboxCount >= 2) {
      ...
      const bulkButton = page.getByRole('button').filter({ hasText: /Manage/i }).first();
      if (await bulkButton.isVisible()) {
        await bulkButton.click();
        await page.waitForTimeout(500);
        
        // Click bulk labels option
        const bulkLabelsItem = page.getByRole('menuitem').filter({ hasText: /Bulk Labels|Labels/i });
        if (await bulkLabelsItem.isVisible()) {
          ...
          if (await keyInput.isVisible() && await valueInput.isVisible()) {
            ...
            // Apply to all
            const applyButton = page.getByRole('button').filter({ hasText: /Apply|Save|Update/i }).first();
