...
test.describe('ITS Cluster Actions Tests', () => {
  ...
  test('edit labels dialog opens and works', async ({ page }) => {
    ...
    if (buttonCount > 0) {
      ...
      if (await editLabelsItem.isVisible()) {
        ...
        const labelValueInput = page.locator('input[placeholder*="value"], input[name*="value"]').first();
  ...
  test('view cluster details dialog opens', async ({ page }) => {
    ...
    if (buttonCount > 0) {
      ...
      const viewDetailsItem = page.getByRole('menuitem').filter({ hasText: /View Details|Details/i });
  ...
  test('detach cluster confirmation dialog works', async ({ page }) => {
    ...
    if (buttonCount > 0) {
      ...
      if (await detachItem.isVisible()) {
        ...
        const confirmButton = page.getByRole('button').filter({ hasText: /Confirm|Yes|Detach/i }).first();
        const cancelButton = page.getByRole('button').filter({ hasText: /Cancel|No/i }).first();
        ...
        if (await cancelButton.isVisible()) {
          ...
          if (await detachItem2.isVisible()) {
            ...
            const confirmButton2 = page.getByRole('button').filter({ hasText: /Confirm|Yes|Detach/i }).first();
